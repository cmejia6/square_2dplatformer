...
public class GeneralMovementNetwork : NetworkBehaviour{
    [SerializeField]
    bool p2Local;
    ...
    public bool CheckCollision(Collision collDir) {
        switch (collDir) {
            ...
            rays = new RaycastHit2D[] {
                Physics2D.Raycast(new Vector2(transform.position.x - collRad, transform.position.y + collRad - 2), transform.up, 2),
                Physics2D.Raycast(new Vector2(transform.position.x + collRad, transform.position.y + collRad - 2), transform.up, 2)
            };
    ...
    protected bool CheckCollision(Collision collDir, float extension) {
        switch (collDir) {
            ...
            rays = new RaycastHit2D[] {
                Physics2D.Raycast(new Vector2(transform.position.x - collRad, transform.position.y + collRad - 2), transform.up, 2 + extension),
                Physics2D.Raycast(new Vector2(transform.position.x + collRad, transform.position.y + collRad - 2), transform.up, 2 + extension)
            };
